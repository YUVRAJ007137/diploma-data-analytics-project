        ...
        <div className="glass-card">
          <div className="flex items-start justify-between">
            <div>
              ...
              <div className="mt-1 muted">{student.enrollmentNumber ? `EN: ${student.enrollmentNumber}` : student.seatNumber ? `Seat: ${student.seatNumber}` : null}</div>
            </div>
            <button type="button" onClick={onClose} className="rounded-md bg-slate-100 px-3 py-1 text-sm text-slate-700 hover:bg-slate-200">Close</button>
          </div>
            ...
            <div className="space-y-2 text-sm text-slate-700">
              {student.semesterBacks && student.semesterBacks.length > 0 ? (
                student.semesterBacks.map((b) => (
                  <div key={b.semester} className="flex items-center justify-between rounded-md glass-tile">
          ...
          <div className="mt-4">
            ...
            <div className="text-2xl font-semibold text-rose-600">{student.totalBacks} backs</div>
          </div>
        </div>
      </div>
    </div>
