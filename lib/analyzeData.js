...
/**
 * Categorize students into three groups based on backlog history:
 * 1. Active backlogs: students with current backlogs
 * 2. Clean/No backlogs: students who never had any backlog
 * 3. Cleared backlogs: students who had backlogs in the past but cleared them
 *
 * @param {object} dataset - { semesters: { ... } }
 * @returns {{ activeBacklog: object[], neverBacklog: object[], clearedBacklog: object[] }}
 */
export function categorizeStudentsByBacklogStatus(dataset) {
  ...
  for (const student of history) {
    const currentBacklog = student.totalBacks || 0;
    const hadBacklogInPast = student.semesterBacks && student.semesterBacks.some(s => s.backCount > 0);

    if (currentBacklog > 0) {
      // Student currently has one or more backlogs
      activeBacklog.push({
        ...student,
        category: 'active',
        statusLabel: `${currentBacklog} active backlog(s)`,
        details: student.semesterBacks
      });
    } else if (!hadBacklogInPast) {
      // Student never had any backlog
      neverBacklog.push({
        ...student,
        category: 'clean',
        statusLabel: 'No backlogs - Clean record',
        details: []
      });
    } else {
      // Student had backlog(s) in past but cleared them all
      const clearedCount = student.semesterBacks
        ?.filter(s => s.backCount > 0)
        .reduce((sum, s) => sum + s.backCount, 0) || 0;

      clearedBacklog.push({
        ...student,
        category: 'cleared',
        statusLabel: `Cleared ${clearedCount} backlog(s)`,
        previousBacklogCount: clearedCount,
        details: student.semesterBacks
      });
    }
  }
  ...
  const sortByName = (a, b) => (a.studentName || '').localeCompare(b.studentName || '');
  activeBacklog.sort(sortByName);
  neverBacklog.sort(sortByName);
  clearedBacklog.sort(sortByName);
